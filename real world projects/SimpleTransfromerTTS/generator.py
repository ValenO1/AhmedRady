...
train_saved_path = r"E:\Grad Project\SimpleTransfromerTTS-master\train_SimpleTransfromerTTS.pt"
state = torch.load(train_saved_path)
model = TransformerTTS().cuda()
model.load_state_dict(state["model"])

text = "hi i am ahmed rady and i want to fuck elsam mohsen so hard from his ass because he is so sexy"
name_file = "output.mp3"


postnet_mel, gate = model.inference(
  text_to_seq(text).unsqueeze(0).cuda(),
  stop_token_threshold=1e-4,
  with_tqdm = False
)
